...
<header class="row" style="border-bottom: 1px solid gray; border-left: 1px solid gray;">
    <div class="col-lg-6">
        <p>
            <?= $main->id; ?>
            <?=
            Html::a('Izmenite zaglavlje stranice:<b>' . $main->$menu_title_name . '</b>', ['/ssslave/update',
                'id' => $main->id,
                'mytype' => '2',
...
<?= $main->$text_name; ?>
    ...
    <nav>
        ...
        <?= Html::a('Dodaj post u ovu stranicu', ['/sssslave/create', 'master_id' => $main->id],
...
<div>
    ...
    if ($main->sssslaves) {
        ...
        <?= $slave->no ?>
                    ...
                    <nav>
                        <?=
                        Html::a('Izmenite post: <b>' . $slave->$menu_title_name . '</b>', ['/sssslave/update',
                            'id' => $slave->id,
                                // 'subpage' => $slave->$link_name,
                                // 'home' => Yii::$app->mycoms->wordtourl($body->home->link_sr_latn),
                                //'subhome' => Yii::$app->mycoms->wordtourl($body->link_sr_latn),
