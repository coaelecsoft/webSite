    ...
    <div class="col-lg-4">
        <div>
            ...
            if (!isset($_GET['master_id'])) {
                if($model->master_id){ ?>
<?= $form->field($model, 'master_id')->textInput(['maxlength' => true, 'readOnly' => true]) ?>            

                    <?php
                }else{
                $allpages = models\Master::find()->all();
                $pages = [];
                foreach ($allpages as $page) {
                    array_push($pages, ['id' => $page->id, 'name' => $page->menu_title_sr_latn]);
                } ?>
   <?= $form->field($model, 'master_id')->dropDownList(ArrayHelper::map($pages, 'id', 'name'), ['prompt' => 'Stranica']); ?> 
                ...
                ?>
                      ...
                      <?php } else { ?>
                <?= $form->field($model, 'master_id')->textInput(['maxlength' => true, 'readOnly' => true, 'value' => $_GET['master_id']]) ?>            
 ...
 <?php
            ...
            if ($model->image1) {
                ...
                <div style="border: 1px gray solid; border-radius: 12px">
                    ...
                    <?= Html::img($mypath . $model->image1, ['alt' => 'myalt', 'title' => 'mytitle', 'class' => 'img_main fadeRotate', 'width' => '90%']) ?>       
                    <?php
                    ?>
                </div> 
            ...
            ?>
            <?= $form->field($model, 'back1')->fileInput() ?> 
