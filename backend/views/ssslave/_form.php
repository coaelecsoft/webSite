    ...
    <div class="col-lg-4">
        <div>
            ...
            if (!isset($_GET['master_id'])) {
                if($model->sub_slave_id){ ?>
<?= $form->field($model, 'sub_slave_id')->textInput(['maxlength' => true, 'readOnly' => true]) ?>            

                    <?php
                }else{
                $allpages = models\Subslave::find()->all();
                $pages = [];
                foreach ($allpages as $page) {
                    array_push($pages, ['id' => $page->id, 'name' => $page->menu_title_sr_latn]);
                } ?>
   <?= $form->field($model, 'sub_slave_id')->dropDownList(ArrayHelper::map($pages, 'id', 'name'), ['prompt' => 'Stranica']); ?> 
                ...
                ?>
                      ...
                      <?php } else { ?>
                <?= $form->field($model, 'sub_slave_id')->textInput(['maxlength' => true, 'readOnly' => true, 'value' => $_GET['master_id']]) ?>            
