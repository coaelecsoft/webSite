            ...
            <article  id="<?= $slave->$link_name ?>" class="my-container">
                <header class="<?= 'text-image-' . $slave->page_option ?>">
                    <div class="text-panel">
                        ...
                        <?php } ?>
                        <?= $slave->$base_name ?>
                        <?php if ($slave->subarticle == '0' and $slave->ssslaves) {///If not be activated showing subarticles panel and if have recorde in $slave->slaves, show button with link to main page of subarticles ?>
                            <nav> 
                                <?=
                                Html::a(' <b>' . $slave->$title_name . '</b>', ['/site/subslave',
                                    'page' => $main->master->$link_name,
                                      'subpage' => $slave->slave->$link_name,
                ...
                <?php
                if ($slave->ssslaves and $slave->subarticle == '1') {
                    ?>

                    <div class="sub-article-container">
                        <?php
                        foreach ($slave->ssslaves as $key => $subslave) {
                            if ($subslave->page_active == "1" and $subslave->menu_option == "1") {
                                ...
                                <article class="sub-article" title="<?= $subslave->$title_name ?>">
                                    ...
                                    <?php if ($subslave->sssslaves/* 1  $slave->slaves */) { ?>
                                        <nav class="link-to-page"> 
                                            <?=
                                            Html::a(' <b>' . $subslave->$title_name . '</b>', ['/site/ssslave',
                                                'page'=> $slave->slave->master->$link_name,
                                                'subpage' => $slave->slave->$link_name,
                                                'subsubpage' => $slave->$link_name,
                                                'subsubsubpage' => $subslave->$link_name,
                                                    // 'home' => Yii::$app->mycoms->wordtourl($body->home->link_sr_latn),
                                                    //'subhome' => Yii::$app->mycoms->wordtourl($body->link_sr_latn),
